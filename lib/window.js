...
const SDL_video = libSDL2('SDL_video')
const SDL_render = libSDL2('SDL_render')
const SDL_rect = libSDL2('SDL_rect')
const SDL_pixels = libSDL2('SDL_pixels')
const SDL_messagebox = libSDL2('SDL_messagebox')
const SDL_keycode = libSDL2('SDL_keycode')
const SDL_keyboard = libSDL2('SDL_keyboard')
...
class window extends EventEmitter {
	constructor(options) {
		...
		options.bgColor = options.bgColor >= 0 ? options.bgColor : 0xFFFFFF
		...
		let change = (w, h) => {
			...
			this.render.color = options.bgColor
			this.render.clear()
		...
		this.on('_event', (event) => {
			if(event.type == SDL_events.SDL_EventType.SDL_WINDOWEVENT) {
				...
				if(evt == SDL_video.SDL_WindowEventID.SDL_WINDOWEVENT_SHOWN) {
					...
					this.emit('moved', event.window.data1, event.window.data2)
				}
	...
	set title(_title) {
		...
		SDL_video.SDL_SetWindowTitle(this._window, _title)
	}
	get title() {
