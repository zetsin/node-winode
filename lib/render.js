...
class render {
	constructor(_render) {
		...
		this.gfx = new gfx.primitives(_render)
	}
	...
	createTexture(w, h, access) {
		access = access >= 0 ? access : 2
	...
	set scale(scaleXY) {
		scaleXY = utils.arraylike2obj(scaleXY, 'x,y')
		SDL_render.SDL_RenderSetScale(this._render, scaleXY.x, scaleXY.y)
	}
	get viewport() {
		let rect = SDL_rect.SDL_Rect()
	...
	static create(window, index, flags) {
		let _render = SDL_render.SDL_CreateRenderer(window, index, flags)
